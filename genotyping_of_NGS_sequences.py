...
ngs_id = input(
    '''This program will take a batch of sequences from a NGS experiment 
    and will perform a genetic classifciation using Maximum Likelihood 
    Clade Assignment method developed in GLUE.\nPlease specify the NGS 
    of interest by typing NGS and number for the experiment (i.e. NGS1978)\n''').upper()
...
with open("/home/phe.gov.uk/juan.ledesma/gluetools/projects/hcv_glue_avu/glue/genotyping_by_genes.js",'r') as program:
    ...
    update = re.sub("source.name like '[A-Z0-9]*%'","source.name like '" + ngs_id + "%'",default_lines)
    program.close()
